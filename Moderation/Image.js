...
data: new SlashCommandBuilder()
	...
	switch (Options) {
		case "glitch": {
			const level = options.getInteger("level") || 3
		...
		case "boil": {
			const level = options.getInteger("level") || 2
		...
		case "hearts": {
			const rainbow = options.getBoolean("rainbow") || false
		...
		case "shear": {
			const axis = options.getString("axis") || "X"
		 ...
		 case "wave": {
		 	const frequency = options.get("frequency").value || 0.05
		 	const amplitude = options.getInteger("amplitude") || 1
